...
void trsm(cublasHandle_t cublasH, long m, long n, double alpha, double* A, long lda, double* B,
          long ldb, long nb) {
    double sonedouble = 1.0, snegonedobule = -1.0;
    if (m <= nb) {
        CUBLAS_CHECK(cublasDtrsm(cublasH, CUBLAS_SIDE_LEFT, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N,
    ...
    long left = m - m / 2;
    CUBLAS_CHECK(cublasDgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, left, n, m / 2, &snegonedobule,
                             A + m / 2, lda, B, ldb, &sonedouble, B + m / 2, ldb));
...
int main(int argc, char* argv[]) {
    ...
    if (check) {
        ...
        CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_B_cublas), sizeof(double) * ldb * n));
        CUDA_CHECK(cudaDeviceSynchronize());
