...
void syr2k(cublasHandle_t cublasH, long n, long k, float alpha, float* A, long lda, float* B,
           long ldb, float beta, float* C, long ldc, long nb) {
    float one = 1;
    long num_block = n / nb;
    long left = n % nb;
    cublasSgemmStridedBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, nb, nb, k, &alpha, A, lda, nb, B,
                              ldb, nb, &beta, C, ldc, nb + nb * ldc, num_block);
    cublasSgemmStridedBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, nb, nb, k, &alpha, B, ldb, nb, A,
                              lda, nb, &one, C, ldc, nb + nb * ldc, num_block);
    if (left > 0) {
        long offset = num_block * nb;
        cublasSgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, left, left, k, &alpha, A + offset, lda,
                    B + offset, ldb, &beta, C + offset + offset * ldc, ldc);
        cublasSgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, left, left, k, &alpha, B + offset, ldb,
                    A + offset, lda, &one, C + offset + offset * ldc, ldc);
    }

    for (long i = 1; i * nb < n; i *= 2) {
        num_block = n / (2 * i * nb);
        left = n - (num_block * 2 * i * nb);
        cublasSgemmStridedBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, i * nb, i * nb, k, &alpha,
                                  A + i * nb, lda, 2 * i * nb, B, ldb, 2 * i * nb, &beta,
                                  C + i * nb, ldc, 2 * (i * nb + i * nb * ldc), num_block);
        cublasSgemmStridedBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, i * nb, i * nb, k, &alpha,
                                  B + i * nb, ldb, 2 * i * nb, A, lda, 2 * i * nb, &one, C + i * nb,
                                  ldc, 2 * (i * nb + i * nb * ldc), num_block);
        if (left > i * nb) {
            long offset_row = i * nb + num_block * (2 * i * nb);
            long offset_col = num_block * (2 * i * nb);
            cublasSgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, left - i * nb, i * nb, k, &alpha,
...
int main(int argc, char* argv[]) {
    ...
    if (check) {
        ...
        CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_C_cublas), sizeof(float) * ldc * n));
        cublasSsyr2k(cublasH, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, k, &one, d_A, lda, d_B, ldb,
                     &zero, d_C_cublas, ldc);
        CUDA_CHECK(cudaStreamSynchronize(stream));
        ...
        cublasSgeam(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, n, n, &sonedouble, d_C, ldc, &snegonedobule,
    ...
    std::cout << "[custom ssyr2k] " << "m: " << n << ", n: " << k << ", "
              << "latency: " << time1 << " ms, " << ((long)n * k * n * 2) / time1 / 1e9 << " TFLOPS"
              << std::endl;
