...
void syrk(cublasHandle_t cublasH, int n, int k, float alpha, float *A, int lda,
    ...
    if (left > 0) {
        ...
        cublasSgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, left, left, k, &alpha, A + offset,
...
int main(int argc, char *argv[]) {
    ...
    CUBLAS_CHECK(cublasSsyrk(cublasH, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, k,
                             &one, d_A, lda, &zero, d_C_cublas, ldc));
    ...
    std::cout << "[custom ssyrk] " << "n: " << n << ", k: " << k << ", "
              << "latency: " << time1 << " ms, " << (long)n * n * k / time1 / 1e9
              << " TFLOPS" << std::endl;
    std::cout << "[cublas ssyrk] " << "n: " << n << ", k: " << k << ", "
