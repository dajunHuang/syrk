...
void syrk(cublasHandle_t cublasH, long n, long k, double alpha, double* A, long lda, double beta,
          double* C, long ldc, long nb) {
    long num_block = n / nb;
    long left = n % nb;
    cublasDgemmStridedBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_T, nb, nb, k, &alpha, A, lda, nb, A,
...
int main(int argc, char* argv[]) {
    ...
    if (check) {
        ...
        cublasDsyrk(cublasH, CUBLAS_FILL_MODE_LOWER, CUBLAS_OP_N, n, k, &one, d_A, lda, &zero,
                    d_C_cublas, ldc);
        CUDA_CHECK(cudaStreamSynchronize(stream));
