...
void trmm(cublasHandle_t cublasH, long m, long n, float alpha, float* A, long lda, float* B,
          long ldb, float beta, float* C, long ldc, long nb) {
    long num_block = m / nb;
    long left = m % nb;
    float one = 1;
    cublasSgemmStridedBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, nb, n, nb, &alpha, A, lda,
    ...
    for (long i = 1; i * nb < m; i *= 2) {
        ...
        cublasSgemmStridedBatched(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, i * nb, n, i * nb, &alpha,
        ...
        if (left > i * nb) {
            ...
            cublasSgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, left - i * nb, n, i * nb, &alpha,
...
int main(int argc, char* argv[]) {
    ...
    if (check) {
        ...
        cublasSgeam(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, m, n, &sonedouble, d_C, ldc, &snegonedobule,
